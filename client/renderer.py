...
class A11yTree():
    ...
    ipc_path: ClassVar[pathlib.Path] = pathlib.Path(os.path.abspath(config.TREE_OUTPUT_PATH))
    ...
    def getElements(cls) -> list[A11yElement]:
        with open(cls.ipc_path) as f:
            ...
            return [A11yElement(element["name"], element["x"], element["y"], element["role"], element["pid"]) for element in raw]
